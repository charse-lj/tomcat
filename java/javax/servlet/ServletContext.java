 ...
 *
 * ServletContext
 *  接口作用范围：一个部署到容器的Web应用都有一个ServletContext接口的实例
 *  初始化参数：允许Servlet访问由应用开发人员在Web应用中的部署描述符中指定的上下文初始化参数 getInitParameter()、getInitParameterNames()
 *  三大件：Servlet、Listener、Filter
 *         添加Servlet或创建Servlet时,@ServletSecurity, @RunAs, @DeclareRoles, @MultipartConfig必须被内省，Filter和Listener不需要内省注解。
 *  上下文属性:Servlet可以使用指定的名字将对象属性绑定到上下文。同一个Web应用内的其他任何Servlet都可以使用绑定到上下文的任意属性。方法允 -->setAttribute、getAttribute、getAttributeNames、removeAttribute
 *  资源：ServletContext接口提供了直接访问Web应用中静态内容层次结构的文件的方法，包括HTML，GIF和JPEG文件
 *       getResource、getResourceAsStream -->getResource和getResourceAsStream方法需要一个以“/”开头的String字符串作为参数，给定的资源路径是相对于上下文的根，或者相对于web应用的WEB-INF/lib目录下的JAR文件中的META-INF/resources目录
 *  临时工作目录:每一个servlet上下文都需要一个临时的存储目录。Servlet容器必须为每一个servlet上下文提供一个私有的临时目录，并将通过javax.servlet.context.tempdir上下文属性使其可用，关联该属性的对象必须是java.io.File类型
 */
public interface ServletContext {
     ...
     *    use this method.
     *    先有描述,然后才能初始化
     *    如果ServletContext传到了ServletContextListener的contextInitialized方法，但该ServletContextListener即没有在web.xml或web-fragment.xml中声明也没有使用@WebListener注解，
     *    则在ServletContext中定义的用于Servlet、Filter和Listener的编程式配置的所有方法必须抛出UnsupportedOperationException
     * @since Servlet 3.0
     */
