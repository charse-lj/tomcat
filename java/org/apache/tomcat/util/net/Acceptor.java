...
public class Acceptor<U> implements Runnable {
    ...
    public void run() {
        ...
        // Loop until we receive a shutdown command  （轮询直到我们收到一个关闭命令）
        while (endpoint.isRunning()) {
            ...
            try {
                //if we have reached max connections, wait  (如果我们已达到最大连接数，请等待)
                endpoint.countUpOrAwaitConnection();

                // Endpoint might have been paused while waiting for latch
                // If that is the case, don't accept new connections
                // 等待latch时Endpoint可能已暂停，如果是这种情况，请不要接受新连接
                ...
                if (endpoint.isRunning() && !endpoint.isPaused()) {
                    ...
                    System.out.println("invoke endpoint#setSocketOptions()");
                    if (!endpoint.setSocketOptions(socket)) {
                        // setSocketOptions()方法如果失败，关闭socket
                        // todo：和下面的destroy有什么区别
