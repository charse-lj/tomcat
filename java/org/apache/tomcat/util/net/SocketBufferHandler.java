        ...
        if (isReadBufferEmpty()) {
            ...
            if (readBufferConfiguredForWrite) {
                ...
                if ((readBuffer.position() + bytesReturned) > readBuffer.capacity()) {
                    ...
                    //readBuffer的position之前的数据,挪到bytesReturned位置后面 -->前面空出bytesReturned位置
                ...
                if (shiftRequired > 0) {
                    ...
                    //limit扩大shiftRequired
                    readBuffer.limit(oldLimit + shiftRequired);
                    for (int i = readBuffer.position(); i < oldLimit; i++) {
                        // i的最小值是position，shiftRequired = bytesReturned - readBuffer.position()-->起始位置:bytesReturned
                        // readBuffer.get(i) --> 获取的是原活跃区的数据
