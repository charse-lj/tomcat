...
public class NioSelectorPool {
    ...
    /**
     * 在池中使用的选择器的最大数量,org.apache.tomcat.util.net.NioSelectorShared默认为false时，使用此选项
     */
    ...
    public Selector get() throws IOException{
        //共享时,使用sharedSelector
        if (shared) {
            return getSharedSelector();
        }
        //独有
        //enabled = false --> 返回null
        //enabled = true && active.incrementAndGet() >= maxSelectors -->返回 null
        ...
        try {
            ...
            if (s == null) {
                //上面与maxSelectors进行对比时+1,如果失败,-1补偿
    ...
    public void put(Selector s) throws IOException {
        ...
        if (enabled && (maxSpareSelectors == -1
                // 空闲数<允许的最大值(maxSpareSelectors与maxSelectors中的最小值)
     ...
     * @param socket        The socket to write data to --> 是Poller现成的socketChannel通道，这个通道是注册在Poller线程中的Selector中,
