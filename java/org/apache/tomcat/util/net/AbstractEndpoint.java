...
public abstract class AbstractEndpoint<S,U> {
    ...
    protected enum BindState {
        UNBOUND,                // 未绑定
        BOUND_ON_INIT,          // 在初始化时绑定了
        BOUND_ON_START,         // 在启动时绑定了
        SOCKET_CLOSED_ON_STOP   // socket在stop后已关闭
    }
     ...
     */
    // 持有的连接，K=底层socket，V=对底层socket的包装
     ...
     * selected the socket.
     *
     * @param socketWrapper The socket wrapper to process        【要处理的socket包装】
     * @param event         The socket event to be processed     【要处理的socket事件】
     * @param dispatch      Should the processing be performed on a new  【是否应在新的容器线程上执行处理】
     *                          container thread
     *
     * @return if processing was triggered successfully  【异步的时候，丢到线程池就算成功了】
     */
    // 以给定状态处理给定的SocketWrapper。 用于触发处理，就好像轮询器（对于具有一个的端点）选择了套接字。
    ...
    public final void init() throws Exception {
        if (bindOnInit) {  // 默认true
            bindWithCleanup();  // 绑定端口，catch异常了就解绑
            bindState = BindState.BOUND_ON_INIT;  // 设置绑定状态
        }
        if (this.domain != null) {
