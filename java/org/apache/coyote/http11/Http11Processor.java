...
public class Http11Processor extends AbstractProcessor {
    ...
    // 解析socket，如果数据读了一半没了，拆包了。
    // 则返回到org.apache.coyote.AbstractProtocol.ConnectionHandler.process中的state == SocketState.LONG流程。
    @Override
    public SocketState service(SocketWrapperBase<?> socketWrapper)
        ...
        while (!getErrorState().isError() && keepAlive && !isAsync() && upgradeToken == null &&
            ...
            try {
                ...
                // 处理请求行的协议组件在尝试解析header之前，需要知道这是否是HTTP 0.9请求。
                ...
                if (protocol.isPaused()) {
                    ...
                    keptAlive = true;
                    // Set this every time in case limit has been changed via JMX  （每次通过JMX更改限制时设置此值）
                    request.getMimeHeaders().setLimit(protocol.getMaxHeaderCount());
                    // Don't parse headers for HTTP/0.9  (不要解析HTTP/0.9的header)
                    /**
                     * 解析header
                     */
            ...
            // Has an upgrade been requested?  （是否要求升级？）
