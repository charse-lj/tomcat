 ...
 */
// Context的启动事件侦听器，用于配置该Context的属性以及关联的已定义servlet。
public class ContextConfig implements LifecycleListener {
        ...
        if ((context instanceof StandardContext) && ((StandardContext) context).getAntiResourceLocking()) {
            ...
            if (originalDocBase.toLowerCase(Locale.ENGLISH).endsWith(".war")) {
                ...
                antiLockingDocBase = new File(
                        System.getProperty("java.io.tmpdir"), deploymentCount++ + "-" + docBase);
            }
    ...
    protected void processClasses(WebXml webXml, Set<WebXml> orderedFragments) {
        SelfPrinter.core(this, "processClasses");
    ...
    protected void processServletContainerInitializers() {
        ...
        try {
            // 此处使用的是Tomcat自己实现的 WebappServiceLoader，它使用方式完全同java.util.ServiceLoader
            WebappServiceLoader<ServletContainerInitializer> loader = new WebappServiceLoader<>(context);
            // 找到所有的ServletContainerInitializer
    ...
    protected void processClass(WebXml fragment, JavaClass clazz) {
        ...
        if (null == annotationsEntries || annotationsEntries.length == 0) {
            return;
        }
        //类名.
    ...
    private void populateJavaClassCache(String className, JavaClass javaClass,
        ...
        //add this super class to the cache
        populateJavaClassCache(javaClass.getSuperclassName(), javaClassCache);
        //add this interfaces to the cache
