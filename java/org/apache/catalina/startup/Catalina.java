...
/**
 * Tomcat 作为web容器需要解决的问题
 * 1.一个web容器可能需要部署两个应用程序，不同的应用程序可能会依赖同一个第三方类库的不同版本，不能要求同一个类库在同一个服务器只有一份，因此要保证每个应用程序的类库都是独立的，保证相互隔离
 * 2.部署在同一个web容器中相同的类库相同的版本可以共享。否则，如果服务器有10个应用程序，那么要有10份相同的类库加载进虚拟机，这是扯淡的
 * 3.web容器也有自己依赖的类库，不能与应用程序的类库混淆。基于安全考虑，应该让容器的类库和程序的类库隔离开来。
 * 4.web容器要支持jsp的修改，我们知道，jsp 文件最终也是要编译成class文件才能在虚拟机中运行，但程序运行后修改jsp已经是司空见惯的事情，否则要你何用？ 所以，web容器需要支持 jsp 修改后不用重启
 *
 * -->jsp 文件其实也就是class文件，那么如果修改了，但类名还是一样，类加载器会直接取方法区中已经存在的，修改后的jsp是不会重新加载的。那么怎么办呢？我们可以直接卸载掉这jsp文件的类加载器，所以你应该想到了，每个jsp文件对应一个唯一的类加载器，当一个jsp文件修改了，就直接卸载这个jsp类加载器。重新创建类加载器，重新加载jsp文件
 *
 *          BootStrap
 *              |
 *          Extension
 ...
 *         Common ClassLoader
 *         /             \
 * Catalina ClassLoader Shard ClassLoader
 *                            |
 *                      WebApp ClassLoader
 *                            |
 *                      JasperLoader
...
public class Catalina {
    ...
    public void setServer(Server server) {
        System.out.println("begin setServer:");
        this.server = server;
    }
    ...
    protected Digester createStartDigester() {
        ...
        /**
         * 拿这个举例，这个地方就会调用Service.addConnector方法，建立Service和Connector的联系
         */
    ...
    public void load() {
        ...
        // Stream redirection  （流重定向：用自定义PrintStream替换System.out和System.err）
        initStreams();

        // Start the new server
        try {

            //层级关系会根据 /conf/server.xml 进行组织
            getServer().init();
        } catch (LifecycleException e) {
    ...
    protected void initNaming() {
        ...
        if (!useNaming) {
            ...
            //向系统属性里设置catalina.useNaming =true
            ...
            //向系统属性里设置java.naming.factory.url.pkgs
