...
/**
 * 该工厂类建造一个为Catalina服务的类加载。
 * factory的方法需要以下参数才能构建新的类加载器（在所有情况下均具有适当的默认值）：
 *      - 一组包含开箱的类（和资源）的目录，应该包含在类加载器的仓库中。
 *      - 一组目录，其中包含JAR文件中的类和资源。 在这些目录中发现的每个可读的JAR文件都将添加到类加载器的仓库中。
 *      - 应该成为新类加载器的父加载器的ClassLoader实例。
 */
