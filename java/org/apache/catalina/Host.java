 ...
 */
/**
 * Host是一个容器，它表示Catalina servlet engine虚拟主机。 在以下类型的方案中很有用：
 *      - 要使用Interceptors看到这个特定的虚拟主机处理每一个请求。
 *      - 你想在一个独立的HTTP connector运行Catalina，但还是想为多台虚拟主机提供支持。
 *
 * 在一般情况下，你不会使用Host时部署Catalina连接到Web服务器（如Apache），
 * 因为Connector将已经利用Web服务器的设施，以确定哪些Context（或者甚至其Wrapper）应该用来处理此请求。
 *
 * Host的父容器通常是Engine，但也可以是一些其他的实现，或者如果没有必要的话也可以省略。
 *
 * Host的子容器通常是Context的实现（代表独立的servlet上下文）。
 */
