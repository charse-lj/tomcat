...
/**
 * 一个Server元素表示整个Catalina servlet容器。 其属性代表了servlet容器作为一个整体的特性。
 * 一个Server可以包含一个或多个Services ，并命名资源的顶级集。
 *
 * 通常情况下，这个接口的实现还将实现Lifecycle ，这样，当start()和stop()方法被调用，所有定义的Services也被启动或停止。
 *
 * 在这两者之间，Server实现类必须在指定的端口号上打开一个服务器socket，端口号记录在port属性上。
 * 当一个连接被accepted时，第一行被读出并与指定的shutdown命令进行比较。 如果命令匹配，启动服务器关闭。
 */
