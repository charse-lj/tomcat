 ...
 *
 * Wrapper的实现是负责管理servlet的生命周期，其底层的servlet类，包括在适当的时间调用init()和destroy()方法，
 * 以及尊重的servlet类本身了SingleThreadModel声明的存在。
 *
 * Wrapper的父容器通常是一个Context的实现类，表示servlet上下文（以及因此该web应用程序）在这个servlet内执行。
 *
 * Wrapper不应该包含子容器，所以addChild()方法应该抛出IllegalArgumentException 。
 */
