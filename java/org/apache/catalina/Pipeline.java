...
/**
 * 该接口描述Valves的集合应当在invoke()方法被调用时，按顺序被执行。
 * 它要求在管道（通常是最后一个）一个Valve的地方必须处理request，并创建相应的response，而不是放过这个request。
 *
 * 通常存在与每个Container相关联的单个Pipeline实例。
 * 容器的正常请求处理功能通常封装在一个容器特定Valve，其应该总是在pipeline的末端被执行。
 * 为了实现这个， 提供了一个setBasic()方法来设置，这个basic将始终是最后一个执行的Valve实例。
 * 其他Valves将以添加的顺序执行，在执行basic Valve前。
 */
