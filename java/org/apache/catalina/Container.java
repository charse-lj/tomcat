...
/**
 * Container(容器)是一个可以执行从客户端接收requests，并返回基于这些请求的responses的对象。
 * 容器还可以通过实现Pipeline接口，来可选地支持以运行时配置的顺序处理请求的Valve管道。
 *
 * 容器将在Catalina的几个概念层次上存在。 以下示例代表常见情况：
 * - Engine - 表示整个Catalina servlet引擎，最有可能包含一个或多个子容器是Host或Context的实现，或其它自定义组。
 * - Host - 表示包含许多Context的一个虚拟主机。
 * - Context - 表示一个单一的ServletContext，其通常将包含所支持的servlets的一个或多个Wrappers。
 * - Wrapper - 表示单独的servlet定义（其可以支持多个Servlet实例如果servlet本身实现了SingleThreadModel）。
 *
 * Catalina的给定部署不需要包括上述所有级别的容器。 例如，管理应用程序的嵌入式的网络设备（如路由器）内可能只包含单个Context与少量的Wrappers，或甚至单个Wrappers，如果应用程序是非常小的话。 因此，容器的实现类需要设计，使他们正确地在无父容器的部署环境下工作。
 *
 * 容器还可以与许多支持组件相关联，这些支持组件提供的功能可以共享（通过将其附加到父容器）或单独定制。 当前公认以下支持组件：
 * - Loader - 类加载器：用于将此Container的新Java类集成到运行Catalina的JVM当中。
 * - Logger - ServletContext 接口的 log()方法签名的实现。
 * - Manager - 与此容器关联的会话池的管理器。
 * - Realm - 安全域的只读接口，用于验证用户身份及其相应角色。
 * - Resources - JNDI目录上下文支持对静态资源的访问，当Catalina嵌入在较大的服务器中时，启用对现有服务器组件的自定义链接。
 */
