...
public abstract class LifecycleMBeanBase extends LifecycleBase
     ...
     * overriding method.
     *
     * 希望执行其他初始化的子类应覆盖此方法，确保super.initInternal()是在重写方法的第一个呼叫。
     */
    @Override
    protected void initInternal() throws LifecycleException {
        // If oname is not null then registration has already happened via preRegister().
        // 如果oname不为null，则表示已经通过preRegister()进行了注册。
        if (oname == null) {
            // 这个字段废弃了
            mserver = Registry.getRegistry(null, null).getMBeanServer();

            // getObjectNameKeyProperties()返回的字符串为："type=Server"
     ...
     */
    /**
     * 实用方法，使子类来轻松地注册不执行附加组件JmxEnabled与MBean服务器。
     * 注意：此方法只使用一次initInternal()被调用之前destroyInternal()被调用。
     *
     * @param obj 要注册的对象
     * @param objectNameKeyProperties  对象名称的关键属性组件，用于注册对象
     * @return 用于注册对象的名称
     */
    // todo
