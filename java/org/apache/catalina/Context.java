...
/**
 * Context是一个容器，表示servlet上下文，并且因此单独的web应用程序，在Catalina servlet engine。
 * 因此，在Catalina（几乎每一个部署，即使连接到Web服务器（如Apache），一个连接器将使用Web服务器的机构，以确定适当的包装来处理这个请求非常有用。
 * 它还提供了一个方便的机制来使用Interceptors看到这个特定的Web应用程序处理的每一个请求。
 *
 * Context的父容器通常是一个Host，也可以是一些其他的实现，或者如果没有必要的情况下也可以省略。
 *
 * Context的子容器通常是Wrapper（表示各个Servlet定义）的实现。
 */
